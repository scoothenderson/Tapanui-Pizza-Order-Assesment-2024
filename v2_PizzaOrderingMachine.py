...
def order_info(): 
    ...
    if is_deliv == 'y':
        ...
        while True:
            ...
            cusmer_addr = input(QUESTION_SET[2]).lower()
    ...
    while True:
        ...
        cusmer_name = input(QUESTION_SET[4]).upper()
...
#Function that prints final receipt
def receip_print(order, is_deliv, detail):
    ...
    for cost in range(1, len(order), 2):
        ...
        #appends each price to a price list
        prices.append(order[cost])
        
        #adds each pizza to the total cost
    ...
    for item in range(0, len(order), 2):
        ...
        #appends each pizza flavour to a list
        flavours.append(order[item])
    
    print('------------------------')
    print('Total pizza(s) ordered: ' + str(len(flavours)))
    print('------------------------')
    ...
    for count in range(0, len(flavours)):
        ...
        #prints out each flavour with it's corresponding price
        print(flavours[count], '- $' + prices[count])
    
    if is_deliv == True:

        #adds a delivery surcharge if the order is a delivery 
        print()
    ...
    #prints customer address, phone number, customer name if order is a delivery
    if is_deliv == True:
        print('Customer Address: ' + (detail[0]) + '\n')
        print('Customer Phone number: ' + '0' + str(detail[1]) + '\n')
        print('Order name: ' + (detail[2]))
        print('------------------')
    
    #prints customer name
    elif is_deliv == False:
        print('Order name: ' + (detail[0]))
        print('------------------')
    
    #confirms if order was correct or not

    while confirm not in ['y', 'n', 'c']:
        confirm = input("ORDER CORRECT? (Y/N), or 'c' TO CANCEL ORDER: ").lower()

        if confirm == 'y':
            return True
        
        if confirm == 'n' or confirm == 'c':
            return False
   ...
   if open_quest == 'q':
     ...
     break
      ...
      if order_info() == False:
       ...
       if receip_print(pizza_order, delivery, cusmer_info) == False:
